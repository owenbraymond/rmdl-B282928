...
rmdl_data <- bind_rows(data_lst, .id = "participant_num") %>%
  filter(condition2 != "congruent" | is.na(condition2)) #Since we are studying the inhibitory effect of word-color pairs, so we are only going to look at incongruent and control trials.
...
```{r}
#The code will print the figures, but you can find a saved vector graphic version in the 'figures' subfolder. 
rawdata_violin <- rmdl_data_RT %>%
  ...
  labs(x = "English Level", y = "RT (ms)", title = "RT by L1/L2 status and color rarity", subtitle="Only color items considered")
...
rawdata_L1_color_noncolor_condition <- rmdl_data_RT %>%
  ...
  scale_color_brewer(name = "Color-Noncolor Condition", labels = c("Color", "Noncolor"), palette = "Set1", type = "div", guide="none") +
  labs(x = "Color and Non-Color Condition", y = "RT (ms)", title = "RT by Color-Noncolor Condition for L1 English speakers", subtitle="Rare and common items considered")
...
rawdata_L1_color_condition <- rmdl_data_RT %>%
  ...
  geom_jitter(width = 0.2, alpha = 0.6, size = 1.4) +
  scale_color_brewer(name = "Rarity Condition", labels = c("common", "rare"), palette = "Set1", type = "div") +
  labs(x = "Zipf Score (SUBTLEX UK)", y = "RT (ms)", title = "RT by Zipf Score (SUBTLEX UK) for L1 English speakers", subtitle="Only color items considered")
...
rmdl_data_bm <- brm(
  RT ~ group * rarity * color_noncolor + Length + (rarity + color_noncolor + Length | participant_num), 
  data = rmdl_data_RT,
  family = lognormal,
  iter = 4000,
  cores = 4,
...
H2_epreds_plot <- draws_combined_epred_H2 %>%
  ...
  theme(axis.text.y = element_text(angle = 45, hjust = 1)) +
  scale_fill_brewer(na.translate = FALSE, name="Credible Intervals", labels = c("95%", "80%", "50%")) + 
  labs(title="Expected Log-RT Reaction Time Predictions for English L1 Speakers", subtitle="Between Rare and Common Color Words",x="Predicted Reaction Times (Log-RTs)", y="Condition")
...
draws_combined_epred_H3 <- rmdl_data_bm_draws_combined %>%
  ...
  #Combining all color responses while keeping L1 and L2 separate to compare color responses between groups
  mutate(
    cond = case_when(
...
draws_combined_epred_H4 %>%
  ...
  theme(axis.text.y = element_text(angle = 45, hjust = 1)) +
  labs(title="Expected Log-RT Reaction Time Predictions for English L1 and L2 Speakers", subtitle="Between Rare and Common Color Words",x="Predicted Reaction Times (Log-RTs)", y="Condition")
```
